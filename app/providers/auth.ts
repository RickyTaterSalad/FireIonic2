...
/*
  ...
  */
  loginWithGoogleUsingPluginPromise:Function = function() {
      ...
      window['plugins'].googleplus.login(
        {
          ...
          'webClientId': '391086405874-gjtqno9ukmok0vupu96jhvlhkm02bbjn.apps.googleusercontent.com',
          'offline': true, // optional, but requires the webClientId - if set to true the plugin will also return a serverAuthCode, which can be used to grant offline access to a non-Google server
        },
        function (authData) {
        //  resolve('got google auth data:' + JSON.stringify(authData, null, 2));
          let provider = firebase.auth.GoogleAuthProvider.credential(authData.idToken, authData.accessToken);
          firebase.auth().signInWithCredential(provider).then((success) => {
            resolve("success");
          }, (error) => {
            resolve("err2" + JSON.stringify(error, null, 2));
          });

        },
        function (msg) {
          resolve("DIDNTWORK: " + msg);
          this.error = msg;
        }
      );
    });
  ...
  /*
  loginWithGoogleUsingPlugin:Function = function() {
    return Observable.create(observer => {
      // note for iOS the googleplus plugin requires ENABLE_BITCODE to be turned off in the Xcode
      window['plugins'].googleplus.login(
          {
